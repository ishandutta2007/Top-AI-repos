...
def get_github_stars(repo_slug):
    ...
    try:
        ...
        # Update cache
        star_cache[repo_slug] = {"stars": stars, "timestamp": now.isoformat()}
        return stars
...
def sort_markdown_table(markdown_content):
    ...
    if next_heading_match:
        table_body_end_index = (
            search_start_for_next_heading + next_heading_match.start()
        )
    else:
        table_body_end_index = len(
            markdown_content
        )  # If no next heading, table goes to end of file
    ...
    original_separator_line = markdown_content[
        separator_match.start() : separator_match.end()
    ]

    # Extract table body rows
    table_body_raw = markdown_content[
        separator_match.end() : table_body_end_index
    ].strip()
    ...
    repo_slug_regex = re.compile(
        r'src="https://custom-icon-badges.herokuapp.com/github/stars/([^/]+/[^?]+)\?'
    )
    ...
    for i, row in enumerate(rows):
        ...
        repo_stars_column_content = cols[
            3
        ]  # This is the content of the Repo_Stars column
        ...
        parsed_repos.append(
            {"original_row": row, "stars": stars, "repo_slug": repo_slug}
        )
    ...
    new_full_table_block = (
        original_header_line + original_separator_line + new_table_body + "\n"
    )

    # Construct the full new markdown content
    new_markdown_content = (
        content_before_table + new_full_table_block + content_after_table
    )
