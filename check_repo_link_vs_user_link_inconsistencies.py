...
def check_table_consistency(file_path="README.md"):
    ...
    for row_id, repo_col, repo_stars_col, user_stars_col in rows:
        ...
        href_match = re.search(
            r'href="https://github\.com/([^/"]+)/?([^/"]+)?"', repo_stars_col
        )
        if href_match:
            ...
            if h_owner != owner or (h_repo and h_repo != repo_name):
                print(
                    f"Row {row_id}: [Repo_Stars] href mismatch. Expected {owner}/{repo_name}, found {h_owner}/{h_repo}"
                )
                mismatches += 1

        # 2. Check badge API URL
        badge_url_match = re.search(
            r'url=https://api\.github\.com/repos/([^/&"]+)/([^/&"]+)', repo_stars_col
        )
        if badge_url_match:
            b_owner, b_repo = badge_url_match.groups()
            if b_owner.lower() != owner or b_repo.lower() != repo_name:
                print(
                    f"Row {row_id}: [Repo_Stars] badge API mismatch. Expected {owner}/{repo_name}, found {b_owner}/{b_repo}"
                )
                mismatches += 1

        # --- Check User_Stars column ---
        # 1. Check anchor tag href
        u_href_match = re.search(
            r'href="https://github\.com/(?:orgs/)?([^/?"]+)', user_stars_col
        )
        if u_href_match:
            u_owner = u_href_match.group(1).lower()
            if u_owner != owner:
                print(
                    f"Row {row_id}: [User_Stars] href mismatch. Expected owner {owner}, found {u_owner}"
                )
                mismatches += 1

        # 2. Check badge API URL
        u_badge_url_match = re.search(
            r'url=https://api\.github-star-counter\.workers\.dev/user/([^/&"]+)',
            user_stars_col,
        )
        if u_badge_url_match:
            ...
            if u_b_owner != owner:
                print(
                    f"Row {row_id}: [User_Stars] badge API mismatch. Expected owner {owner}, found {u_b_owner}"
                )
                mismatches += 1
